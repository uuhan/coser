...
#define LOG_LEVEL_STRING(level)                                        \
    (COS_LOG_DBG == (level & COS_LOG_DBG)                              \
         ? "[DBG] "                                                    \
         : COS_LOG_INFO == (level & COS_LOG_INFO)                      \
               ? "[INFO] "                                             \
               : COS_LOG_WARN == (level & COS_LOG_WARN)                \
                     ? "[WARN] "                                       \
                     : COS_LOG_ERR == (level & COS_LOG_ERR) ? "[ERR] " \
                                                            : "[CRIT]")
...
#define SDK_LOG_DBG(fmt, ...) \
    COS_LOW_LOGPRN(COS_LOG_DBG, fmt, ##__VA_ARGS__, "")
#define SDK_LOG_INFO(fmt, ...) \
    COS_LOW_LOGPRN(COS_LOG_INFO, fmt, ##__VA_ARGS__, "")
#define SDK_LOG_WARN(fmt, ...) \
    COS_LOW_LOGPRN(COS_LOG_WARN, fmt, ##__VA_ARGS__, "")
#define SDK_LOG_ERR(fmt, ...) \
    COS_LOW_LOGPRN(COS_LOG_ERR, fmt, ##__VA_ARGS__, "")
#define SDK_LOG_COS(level, fmt, ...) \
    COS_LOW_LOGPRN(level, fmt, ##__VA_ARGS__, "")
