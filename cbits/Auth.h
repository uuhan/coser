...
public:
    /// @brief
    /// 返回多次有效的签名，可以在有效期内重复对多个资源使用，以下参数含义均可在qcloud官网上
...
const char* appSignMulti(uint64_t app_id, const char* secret_id,
                         const char* secret_key, uint64_t expired_time_in_s,
                         const char* bucket_name);
const char* appSignOnce(uint64_t app_id, const char* secret_id,
                        const char* secret_key, const char* path,
                        const char* bucket_name);
#endif  // AUTH_H
