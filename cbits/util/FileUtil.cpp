...
{
string
FileUtil::getFileContent(const string &localFilePath)
{
    std::ifstream fileInput(localFilePath.c_str(),
...
uint64_t
FileUtil::getFileLen(const string &localFilePath)
{
    std::ifstream fileInput(localFilePath.c_str(),
                            std::ios::in | std::ios::binary);
...
bool
FileUtil::isFileExists(const string &filename)
{
...
string
FileUtil::FormatFolderPath(const string &path)
{
...
string
FileUtil::FormatFilePath(const string &path)
{
...
bool
FileUtil::IsLegalFilePath(const string &path)
{
    size_t len = path.length();
    if (len == 0 || path[0] != kPathDelimiterChar ||
        path[len - 1] == kPathDelimiterChar) {
        return false;
    } else {
        return true;
    }
...
bool
FileUtil::isValidFolderName(const string &foldername)
{
    ...
    //判断名称中是否包含保留字符'/' , '?' , '*' , ':' , '|' , '\' , '<' , '>' ,
    //'"'
    static const char reserve_char[] = {'/',  '?', '*', ':',  '|',
                                        '\\', '<', '>', '\"', '\0'};
    static const int reserve_char_len =
        sizeof(reserve_char) / sizeof(reserve_char[0]) - 1;
...
bool
FileUtil::isLegalFolderPath(const string &path)
{
    size_t len = path.length();
    //文件夹前后都要带'/'
    if (len == 0 || path[0] != kPathDelimiterChar ||
...
bool
FileUtil::IsRootPath(const string &dstPath)
{
    return (dstPath == kPathDelimiter);
}

string
FileUtil::EncodePath(const string &dstPath)
{
    string encodeStr = "";
    if (dstPath.empty()) {
        return encodeStr;
...
uint64_t
FileUtil::GetExpiredTime()
{
    return time(NULL) + CosSysConfig::getExpiredTime();
}

string
FileUtil::FormatPath(const string &path)
{
    if (path.empty()) {
        return path;
    }
    ...
    for (size_t i = 0; i < len; ++i) {
        if (path[i] == kPathDelimiterChar && last_delimiter_pos + 1 == i) {
            ...
            continue;
        } else if (path[i] == kPathDelimiterChar &&
                   last_delimiter_pos + 1 < i) {
            last_delimiter_pos = i;
