...
{
unsigned char
CodecUtil::ToHex(const unsigned char &x)
{
    return x > 9 ? (x - 10 + 'A') : x + '0';
}

string
CodecUtil::UrlEncode(const string &str)
{
    string encodedUrl  = "";
...
string
CodecUtil::Base64Encode(const string &plainText)
{
...
string
CodecUtil::HmacSha1(const string &plainText, const string &key)
{
...
string
CodecUtil::GetFileSha1(const string &localFilePath)
{
...
string
CodecUtil::GetFileSha1(const char *buffer, size_t buff_len)
{
    SHA_CTX sc;
    SHA1_Init(&sc);
