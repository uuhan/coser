...
{
Sha1::Sha1() { sha_init(&m_sha); }
Sha1::~Sha1() {}
void
Sha1::append(const char *data, unsigned int size)
{
...
void
sha_stream(unsigned char digest[20], SHA_INFO *sha_info, FILE *fin, int isize)
{
    int i;
...
/* print a SHA digest */
void
sha_print(unsigned char digest[20])
{
    int i, j;
...
void
sha_output(unsigned char digest[20], unsigned char output[40])
{
    int i = 0;
    for (i = 0; i < 20; i++)
